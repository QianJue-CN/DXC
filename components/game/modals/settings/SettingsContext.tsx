                ...
                <div className={`flex-1 flex flex-col bg-white overflow-hidden transition-all duration-300 ${isPreviewExpanded ? 'w-full' : ''}`}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 bg-zinc-50 border-b border-zinc-200">
                            {activeModule ? (
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="border border-zinc-200 rounded overflow-hidden bg-white shadow-sm">
                                                    <div className="bg-zinc-100 px-3 py-2 border-b border-zinc-200 text-xs font-bold text-zinc-500 flex justify-between">
                                                        <span>NPC 状态覆盖 (Direct Override)</span>
                                                    ...
                                                    <div className="max-h-60 overflow-y-auto custom-scrollbar">
                                                        <table className="w-full text-xs text-left">
                                                            <thead className="bg-zinc-50 text-zinc-400 font-medium sticky top-0">
                                                                <tr>
                                                                    <th className="p-2 font-light">姓名</th>
                                                                    <th className="p-2 text-center font-light">在场</th>
                                                                    <th className="p-2 text-center font-light">关注</th>
                                                                    <th className="p-2 text-center font-light">队友</th>
                                                                    <th className="p-2 text-center font-light">强制上下文</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {gameState.社交.map(c => (
                                ...
                                <div className="h-full flex flex-col items-center justify-center text-zinc-400">
                                    ...
                                    <p className="text-xs uppercase font-bold tracking-widest">请选择要配置的模块</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div className={`${isPreviewExpanded ? 'flex-1 h-full' : 'h-48'} bg-zinc-900 border-t border-zinc-800 flex flex-col transition-all duration-300 relative`}>
                        <div className="flex justify-between items-center p-2 bg-black text-zinc-500 text-[10px] font-bold uppercase tracking-wider shrink-0">
                            <span className="flex items-center gap-2"><Code size={12} /> 输出预览</span>
                            <div className="flex gap-2 items-center">
                                <button onClick={() => setPreviewMode('MODULE')} className={previewMode === 'MODULE' ? 'text-indigo-400' : 'hover:text-white'}>模块</button>
                                <span>|</span>
                                <button onClick={() => setPreviewMode('FULL')} className={previewMode === 'FULL' ? 'text-green-400' : 'hover:text-white'}>完整上下文</button>
                                <div className="w-px h-3 bg-zinc-700 mx-1" />
                                <button onClick={() => setIsPreviewExpanded(!isPreviewExpanded)} className="hover:text-white text-zinc-300">
                                    {isPreviewExpanded ? (
                                        <div className="flex items-center gap-1 bg-zinc-800 px-2 py-0.5 rounded"><Minimize2 size={12} /> 退出全屏</div>
                                    ) : (
                                        <div className="flex items-center gap-1"><Maximize2 size={12} /> 展开</div>
                                    )}
                                </button>
                            </div>
                        </div>
