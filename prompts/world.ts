...
?? **核心指令**: 当玩家进入新的地下城楼层，且该楼层数据为空时，**必须**生成该楼层完整拓扑数据。
?? **触发条件**: 仅在玩家位于地下城时触发；并且楼层应与 \`gameState.地图.current.floor\` 一致。
?? **坐标基准**: 地下城使用**像素坐标**，必须落在 \`gameState.地图.dungeons[DungeonId].floors[Floor].bounds\` 范围内。
